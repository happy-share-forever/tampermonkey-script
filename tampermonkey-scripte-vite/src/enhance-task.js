...
export function enhanceTask (document, projectPrefix, urlDomain) {
  ...
  target.each(function () {
    ...
    $copyTitle.on('click', function () {
      let title = window.location.search.includes('f=bug') ? $($el.children()[3]).attr('title') : $(document).find(`tr[data-id=${taskId}]`).find('.c-name').attr('title')
